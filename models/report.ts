...
export interface IReportDocument extends Document {
  ...
  createdAt: Date;
  updatedAt: Date;
  generateReport(userId: mongoose.Types.ObjectId): Promise<any>;
  scheduleReport(frequency: string, recipients: string[]): IReportDocument;
...
export interface IReportModel extends Model<IReportDocument> {
  getReportsByType(type: string, userId: mongoose.Types.ObjectId): Promise<IReportDocument[]>;
  getScheduledReports(userId: mongoose.Types.ObjectId): Promise<IReportDocument[]>;
  getReportTemplates(type: string): any;
  createDefaultReports(userId: mongoose.Types.ObjectId): Promise<any>;
}
...
reportSchema.methods.generateReport = async function(userId) {
  ...
  try {
    const reportData = await (ReportGenerator as any).generateReport(this);
...
reportSchema.statics.createDefaultReports = async function(userId) {
  ...
  for (const report of Object.values(templates)) {
    await (new this(report) as any).save();
  }
  
  return templates;
};

export default mongoose.model<IReportDocument, IReportModel>("Report", reportSchema);
