...
interface IUserDocument extends Document {
  ...
  subscription: "free" | "basic" | "lite" | "business";
  termsAndConditionsAccepted: boolean;
  wallet?: mongoose.Types.ObjectId;
  virtualAccount?: mongoose.Types.ObjectId;
  savingsPlans: mongoose.Types.ObjectId[];
  kycStatus: "not_started" | "pending" | "verified" | "rejected";
  kycDocuments: Array<{
    type: "id_card" | "utility_bill" | "passport" | "selfie";
    url: string;
  ...
  createdAt: Date;
  updatedAt: Date;
  lockAccount(hours?: number): Promise<IUserDocument>;
  unlockAccount(): Promise<IUserDocument>;
  incrementLoginAttempts(): Promise<IUserDocument>;
}

// Model interface (static methods)
interface IUserModel extends Model<IUserDocument> {
  findByReferralCode(referralCode: string): Promise<IUserDocument | null>;
  getUserStats(userId: mongoose.Types.ObjectId): Promise<any>;
} 
...
userSchema.methods.lockAccount = function(this: IUserDocument, hours = 24) {
...
userSchema.methods.unlockAccount = function(this: IUserDocument) {
  this.accountLocked = false;
  this.lockedUntil = null;
  this.loginAttempts = 0;
  return this.save();
...
userSchema.statics.findByReferralCode = function(this: IUserModel, referralCode) {
  return this.findOne({ referralCode: referralCode.toUpperCase() });
}

// Static method to get user statistics
userSchema.statics.getUserStats = async function(this: IUserModel, userId) {
