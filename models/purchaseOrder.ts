...
export interface IPurchaseOrderDocument extends Document {
  ...
  calculateTotals(): IPurchaseOrderDocument;
  receiveItems(receivedItems: any[], receivedBy: mongoose.Types.ObjectId): Promise<IPurchaseOrderDocument>;
  sendOrder(approvedBy: mongoose.Types.ObjectId): Promise<IPurchaseOrderDocument>;
}

// Model interface (static methods)
export interface IPurchaseOrderModel extends Model<IPurchaseOrderDocument> {
  getPendingOrders(userId: mongoose.Types.ObjectId): Promise<IPurchaseOrderDocument[]>;
  searchOrders(query: string, userId: mongoose.Types.ObjectId, filters?: any): any;
}
...
// orderNumber already has unique: true, so no need for additional index
...
purchaseOrderSchema.pre("save", async function(next: () => void) {
  if (!this.orderNumber) {
    const count = await (this.constructor as IPurchaseOrderModel).countDocuments();
    const date = new Date();
...
export default mongoose.model<IPurchaseOrderModel>("PurchaseOrder", purchaseOrderSchema);
