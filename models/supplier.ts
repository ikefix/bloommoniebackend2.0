...
export interface ISupplierDocument extends Document {
  ...
  addProduct(productData: any): Promise<ISupplierDocument>;
  updateBalance(amount: number, type?: string): Promise<ISupplierDocument>;
}

// Model interface (static methods)
export interface ISupplierModel extends Model<ISupplierDocument> {
  searchSuppliers(query: string, userId: mongoose.Types.ObjectId): any;
  getSuppliersWithProducts(userId: mongoose.Types.ObjectId): Promise<ISupplierDocument[]>;
}
