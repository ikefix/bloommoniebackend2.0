...
export interface IStaffDocument extends Document {
  ...
  createdAt: Date;
  updatedAt: Date;
  lockAccount(hours?: number): Promise<IStaffDocument>;
  unlockAccount(): Promise<IStaffDocument>;
  incrementLoginAttempts(): Promise<IStaffDocument>;
  updatePerformance(performanceData: any): Promise<IStaffDocument>;
}
...
export interface IStaffModel extends Model<IStaffDocument> {
  ...
  getStaffSalesPerformance(startDate: Date, endDate: Date): any;
  searchStaff(query: string, filters?: any): any;
}
...
staffSchema.virtual("employmentDuration").get(function() {
  ...
  const now = new Date();
  return Math.floor((now.getTime() - this.hireDate.getTime()) / (1000 * 60 * 60 * 24 * 30.44)); // months
});
...
export default mongoose.model<IStaffDocument, IStaffModel>("Staff", staffSchema);
