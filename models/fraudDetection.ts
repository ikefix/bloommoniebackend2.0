...
interface IFraudDetectionDocument extends Document {
  ...
  createdAt: Date;
  updatedAt: Date;
  checkRiskFactors(): Promise<void>;
  determineAction(): void;
}

// Model interface (static methods)
interface IFraudDetectionModel extends Model<IFraudDetectionDocument> {
  analyzeTransaction(transactionData: any): Promise<IFraudDetectionDocument>;
  checkVelocityLimits(userId: mongoose.Types.ObjectId, transactionType: string, amount: number): Promise<boolean>;
  getUserRiskProfile(userId: mongoose.Types.ObjectId): Promise<any>;
}
...
export default mongoose.model<IFraudDetectionModel>("FraudDetection", fraudDetectionSchema);
