...
export interface ISavingsPlanDocument extends Document {
  ...
  isMatured?: boolean;
  addContribution(amount: number, reference: string, description?: string): Promise<ISavingsPlanDocument>;
  requestWithdrawal(amount: number, description?: string): Promise<ISavingsPlanDocument>;
  approveWithdrawal(withdrawalId: string, approvedBy: mongoose.Types.ObjectId): Promise<ISavingsPlanDocument>;
  rejectWithdrawal(withdrawalId: string, approvedBy: mongoose.Types.ObjectId): Promise<ISavingsPlanDocument>;
  calculateTransactionFee(amount: number, transactionType: string): number;
  calculateManagementFee(): number;
  checkMaintenanceFee(): any;
  deductFees(): Promise<any>;
  getFeeSummary(): any;
  calculateInterest(): number;
}

// Model interface (static methods)
export interface ISavingsPlanModel extends Model<ISavingsPlanDocument> {
  getUserSavingsPlans(userId: mongoose.Types.ObjectId, status?: string): Promise<ISavingsPlanDocument[]>;
  getMaturedPlans(): Promise<ISavingsPlanDocument[]>;
}
...
// Pre-save middleware
savingsPlanSchema.pre("save", async function() {
...
savingsPlanSchema.methods.calculateInterest = function() {
  ...
  const daysActive = Math.floor((new Date().getTime() - new Date(this.startDate).getTime()) / (1000 * 60 * 60 * 24));
...
export default mongoose.model<ISavingsPlanDocument, ISavingsPlanModel>("SavingsPlan", savingsPlanSchema);
