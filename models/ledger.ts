...
export interface ILedgerEntryDocument extends Document {
  ...
  createdAt: Date;
  updatedAt: Date;
  reverse(reason: string, reversedBy: mongoose.Types.ObjectId): Promise<ILedgerEntryDocument>;
}

// Model interface (static methods)
export interface ILedgerEntryModel extends Model<ILedgerEntryDocument> {
  createDoubleEntry(entries: any[]): Promise<ILedgerEntryDocument[]>;
  getAccountBalance(accountType: string, accountId: mongoose.Types.ObjectId): Promise<any>;
...
ledgerEntrySchema.pre("save", async function(next: any) {
...
export default mongoose.model<ILedgerEntryModel>("LedgerEntry", ledgerEntrySchema);
