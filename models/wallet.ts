...
export interface IWalletDocument extends Document {
  ...
  updatedAt: Date;
  isLocked(): boolean;
  lockWallet(hours?: number): Promise<IWalletDocument>;
  unlockWallet(): Promise<IWalletDocument>;
  incrementLoginAttempts(): Promise<IWalletDocument>;
  checkTransactionLimit(amount: number, transactionType?: string): Promise<boolean>;
  lockFunds(amount: number, reason: string): Promise<any>;
  unlockFunds(amount: number): Promise<any>;
  updateBalance(amount: number, type: string): Promise<IWalletDocument>;
  getSummary(): Promise<any>;
}

// Model interface (static methods)
export interface IWalletModel extends Model<IWalletDocument> {
  createWallet(userId: mongoose.Types.ObjectId, currency?: string): Promise<IWalletDocument>;
  getUserWallet(userId: mongoose.Types.ObjectId): Promise<IWalletDocument>;
  freezeWallet(userId: mongoose.Types.ObjectId, reason: string): Promise<IWalletDocument>;
  unfreezeWallet(userId: mongoose.Types.ObjectId): Promise<IWalletDocument>;
}
...
// Pre-save middleware
walletSchema.pre("save", function(this: IWalletDocument, next: (err?: Error) => void) {
...
walletSchema.methods.getSummary = async function() {
  ...
  const totalSavings = savingsPlans.reduce((sum: number, plan: any) => sum + plan.currentAmount, 0);
  const activeSavings = savingsPlans.filter((plan: any) => plan.status === "active").length;
...
walletSchema.statics.createWallet = async function(this: IWalletModel, userId: mongoose.Types.ObjectId, currency = "NGN") {
...
walletSchema.statics.getUserWallet = async function(this: IWalletModel, userId: mongoose.Types.ObjectId) {
...
walletSchema.statics.freezeWallet = async function(this: IWalletModel, userId: mongoose.Types.ObjectId, reason: string) {
...
walletSchema.statics.unfreezeWallet = async function(this: IWalletModel, userId: mongoose.Types.ObjectId) {
  const wallet = await this.findOne({ userId });
  if (!wallet) {
    throw new Error("Wallet not found");
  }
