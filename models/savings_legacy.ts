...
savingsSchema.pre("save", function(next: any) {
...
savingsSchema.statics.migrateToNewSystem = async function(userId) {
  const legacySavings: any = await this.findOne({ userId, migrated: false });
  
  if (!legacySavings) {
    return null; // Already migrated or doesn't exist
  }

  const SavingsPlan: any = mongoose.model('SavingsPlan');
  const Wallet: any = mongoose.model('Wallet');
  const LedgerEntry: any = mongoose.model('LedgerEntry');
...
savingsSchema.statics.getMigrationStatus = async function(userId) {
  const legacySavings: any = await this.findOne({ userId });
