...
export interface IVirtualAccountDocument extends Document {
  ...
  updatedAt: Date;
  updateTransactionStats(amount: number): Promise<IVirtualAccountDocument>;
  checkTransactionLimit(amount: number, transactionDate?: Date): Promise<boolean>;
}

// Model interface (static methods)
export interface IVirtualAccountModel extends Model<IVirtualAccountDocument> {
  createVirtualAccount(userId: mongoose.Types.ObjectId, accountName: string, provider?: string): Promise<IVirtualAccountDocument>;
  createPaystackVirtualAccount(userId: mongoose.Types.ObjectId, accountName: string): Promise<any>;
  createMonnifyVirtualAccount(userId: mongoose.Types.ObjectId, accountName: string): Promise<any>;
  createFlutterwaveVirtualAccount(userId: mongoose.Types.ObjectId, accountName: string): Promise<any>;
  getUserVirtualAccount(userId: mongoose.Types.ObjectId): Promise<IVirtualAccountDocument | null>;
  deactivateVirtualAccount(userId: mongoose.Types.ObjectId): Promise<any>;
}
...
virtualAccountSchema.statics.createVirtualAccount = async function(this: IVirtualAccountModel, userId, accountName, provider = "paystack") {
...
export default mongoose.model<IVirtualAccountModel>("VirtualAccount", virtualAccountSchema);
