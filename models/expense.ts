...
interface IExpenseDocument extends Document {
  title: string;
  description: string;
  amount: number;
  ...
  totalWithTax?: number;
  isOverdue?: boolean;
  approve(approvedBy: mongoose.Types.ObjectId): Promise<IExpenseDocument>;
  reject(approvedBy: mongoose.Types.ObjectId, reason: string): Promise<IExpenseDocument>;
  reimburse(reimbursedBy: mongoose.Types.ObjectId): Promise<IExpenseDocument>;
  setNextRecurringDate(): Promise<IExpenseDocument>;
}

// Model interface (static methods)
interface IExpenseModel extends Model<IExpenseDocument> {
  getExpenseSummary(userId: mongoose.Types.ObjectId, startDate?: string, endDate?: string): Promise<any[]>;
  getExpensesByCategory(userId: mongoose.Types.ObjectId, startDate?: string, endDate?: string): Promise<any[]>;
  getRecurringExpenses(userId: mongoose.Types.ObjectId): any;
  getPendingExpenses(userId: mongoose.Types.ObjectId): any;
  searchExpenses(query: string, userId: mongoose.Types.ObjectId, filters?: any): any;
}
...
export default mongoose.model<IExpenseDocument, IExpenseModel>("Expense", expenseSchema);
