...
export interface ISaleDocument extends Document {
  ...
  balanceDue?: number;
  isFullyPaid?: boolean;
  calculateTotals(): ISaleDocument;
  addPayment(paymentData: any): Promise<ISaleDocument>;
  processSale(): Promise<ISaleDocument>;
}

// Model interface (static methods)
export interface ISaleModel extends Model<ISaleDocument> {
  getSalesSummary(userId: mongoose.Types.ObjectId, startDate?: string, endDate?: string): Promise<any[]>;
  searchSales(query: string, userId: mongoose.Types.ObjectId, filters?: any): any;
}
...
saleSchema.pre("save", async function() {
  if (!this.invoiceNumber) {
    const count = await (this.constructor as ISaleModel).countDocuments();
    const date = new Date();
...
saleSchema.methods.processSale = async function() {
  ...
  for (const item of this.items) {
    ...
    if ((product as any).stock.currentStock < item.quantity) {
    ...
    await (StockMovement as any).createMovement({
      ...
      unitCost: (product as any).purchasePrice,
      totalCost: (product as any).purchasePrice * item.quantity
    });
    
    // Update product stock
    await (product as any).updateStock(item.quantity, "subtract");
  }
  ...
  if (this.isCreditSale && this.customer) {
    ...
    if (customer) {
      await (customer as any).updateCredit(this.balanceDue, "add");
      
      // Add loyalty points (1 point per ₦10 spent)
      const points = Math.floor(this.totalAmount / 10);
      await (customer as any).addLoyaltyPoints(points);
    }
  }
...
export default mongoose.model<ISaleDocument, ISaleModel>("Sale", saleSchema);
