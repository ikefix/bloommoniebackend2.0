...
export interface IStockMovementModel extends Model<IStockMovementDocument> {
  createMovement(data: any): Promise<IStockMovementDocument>;
  getProductMovements(productId: mongoose.Types.ObjectId, filters?: any): any;
  getStockSummary(productId: mongoose.Types.ObjectId, startDate?: string, endDate?: string): any;
}
...
stockMovementSchema.statics.createMovement = async function(data) {
  const Product: any = mongoose.model('Product');
  ...
  const previousStock = product.stock?.currentStock || 0;
...
stockMovementSchema.statics.getStockSummary = function(productId, startDate, endDate) {
  const matchStage: any = { product: new mongoose.Types.ObjectId(productId) };
...
export default mongoose.model<IStockMovementDocument, IStockMovementModel>("StockMovement", stockMovementSchema);
