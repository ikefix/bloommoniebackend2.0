...
interface IExpenseCategoryDocument extends Document {
  name: string;
  code: string;
  description: string;
...
interface IExpenseCategoryModel extends Model<IExpenseCategoryDocument> {
  getCategoryTree(userId: mongoose.Types.ObjectId): Promise<any[]>;
  getRootCategories(userId: mongoose.Types.ObjectId): Promise<IExpenseCategoryDocument[]>;
  searchCategories(query: string, userId: mongoose.Types.ObjectId): Promise<IExpenseCategoryDocument[]>;
}
...
expenseCategorySchema.virtual("fullPath").get(function() {
  const buildPath = async (category: IExpenseCategoryDocument) => {
    const path = [category.name];
    if (category.parentCategory) {
      const parent = await (this.constructor as Model<IExpenseCategoryDocument>).findById(category.parentCategory);
      if (parent) {
...
export default mongoose.model<IExpenseCategoryDocument, IExpenseCategoryModel>("ExpenseCategory", expenseCategorySchema);
