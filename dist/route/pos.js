...
router.get("/customers/search", auth, async (req, res) => {
    try {
        ...
        const customers = await Customer.searchCustomers(query, new mongoose.Types.ObjectId(req.user._id));
        res.json({
            success: true,
            data: customers
        });
...
router.get("/discounts", auth, async (req, res) => {
    try {
        const discounts = await Discount.findValidDiscounts(new mongoose.Types.ObjectId(req.user._id));
        res.json({
            success: true,
            data: discounts
        });
...
router.post("/cart/discount", auth, async (req, res) => {
    try {
        ...
        const discount = await Discount.getByCode(discountCode, new mongoose.Types.ObjectId(req.user._id));
        if (!discount) {
...
router.post("/checkout", auth, async (req, res) => {
    try {
        const { paymentMethod, paidAmount, isCreditSale = false, creditDueDate = null, notes = "" } = req.body;
        const cart = initializeCart(req.user._id);
...
router.get("/sales/summary", auth, async (req, res) => {
    try {
        ...
        const summary = await Sale.getSalesSummary(new mongoose.Types.ObjectId(req.user._id), startDate, endDate);
        res.json({
            success: true,
            data: summary[0] || {
                totalSales: 0,
...
router.get("/sales", auth, async (req, res) => {
    try {
        ...
        let query = Sale.searchSales(search, new mongoose.Types.ObjectId(req.user._id), filters);
