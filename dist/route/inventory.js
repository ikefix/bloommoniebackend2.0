...
router.delete("/products/:id", auth, async (req, res) => {
    try {
        ...
        product.lastUpdatedBy = new mongoose.Types.ObjectId(req.user._id);
        await product.save();
        res.json({
            success: true,
...
router.post("/categories", auth, async (req, res) => {
    try {
        ...
        if (!categoryData.code) {
            const count = await Category.countDocuments({ assignedShop: new mongoose.Types.ObjectId(shopId) });
...
router.get("/stock/movements/:productId", auth, async (req, res) => {
    try {
        ...
        const pageNum = typeof page === 'number' ? page : parseInt(page) || 1;
        const limitNum = typeof limit === 'number' ? limit : parseInt(limit) || 50;
        const skip = (pageNum - 1) * limitNum;
        const filters = {};
        if (type)
            filters.type = type;
        const movements = await StockMovement.getProductMovements(new mongoose.Types.ObjectId(req.params.productId), filters).skip(skip).limit(limitNum);
...
router.get("/dashboard", auth, async (req, res) => {
    try {
        const [totalProducts, lowStockCount, outOfStockCount, totalCategories, totalBrands, recentMovements] = await Promise.all([
            ...
            Category.countDocuments({ isActive: true, assignedShop: new mongoose.Types.ObjectId(req.query.shopId) }),
