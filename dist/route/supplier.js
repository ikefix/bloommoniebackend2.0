...
router.get("/", auth, async (req, res) => {
    try {
        const { page = 1, limit = 20, search, businessType, rating } = req.query;
        ...
        if (rating !== undefined && rating !== null) {
            ...
            filters.rating = parseInt(ratingValue);
        }
...
router.put("/:id/products/:productId", auth, async (req, res) => {
    try {
        ...
        const product = supplier.products.find((p) => p._id?.toString() === req.params.productId);
        if (!product) {
...
router.post("/purchase-orders/:id/send", auth, async (req, res) => {
    try {
        ...
        await order.sendOrder(new mongoose.Types.ObjectId(req.user._id));
        res.json({
            success: true,
...
router.post("/purchase-orders/:id/receive", auth, async (req, res) => {
    try {
        ...
        await order.receiveItems(receivedItems, new mongoose.Types.ObjectId(req.user._id));
        res.json({
            success: true,
...
router.get("/:id/transactions", auth, async (req, res) => {
    try {
        ...
        const summary = await Invoice.getInvoiceSummary(new mongoose.Types.ObjectId(req.user._id), startDate, endDate);
        res.json({
            success: true,
            data: {
...
router.get("/:id/dashboard", auth, async (req, res) => {
    try {
        ...
        const [supplier, totalOrders, pendingOrders, totalInvoices, overdueInvoices, recentTransactions] = await Promise.all([
