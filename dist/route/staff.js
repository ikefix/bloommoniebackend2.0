...
router.get("/roles/level/:level", auth, async (req, res) => {
    try {
        ...
        const roles = await Role.getRolesByLevel(levelValue);
        res.json({
            success: true,
            data: roles
        });
...
router.get("/performance", auth, async (req, res) => {
    try {
        ...
        let performance = await Staff.getStaffSalesPerformance(startDate, endDate);
        if (department) {
            performance = performance.filter(p => p.department === department);
        }
        res.json({
            success: true,
            data: performance
