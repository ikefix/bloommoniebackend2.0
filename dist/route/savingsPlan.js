...
router.post("/create", auth, async (req, res) => {
    try {
        const { name, description, type, targetAmount, interestRate, maturityDate, autoSave, withdrawalRules } = req.body;
        if (!name || !type) {
...
router.put("/:id", auth, async (req, res) => {
    try {
        ...
        if (name)
            savingsPlan.name = name;
        if (description !== undefined)
            savingsPlan.description = description;
        if (autoSave)
            savingsPlan.autoSave = { ...savingsPlan.autoSave, ...autoSave };
        if (withdrawalRules)
            savingsPlan.withdrawalRules = { ...savingsPlan.withdrawalRules, ...withdrawalRules };
        await savingsPlan.save();
        res.json({
