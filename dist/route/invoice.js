...
router.get("/", auth, async (req, res) => {
    try {
        const { page = 1, limit = 20, search, type, status, customer, supplier, startDate, endDate } = req.query;
...
router.post("/from-sale", auth, async (req, res) => {
    try {
        ...
        const invoice = new Invoice(invoiceData);
        await invoice.calculateTotals();
        await invoice.sendInvoice(new mongoose.Types.ObjectId(req.user._id));
        await Invoice.findById(invoice._id)
            .populate('customer', 'name phone email address')
            .populate('items.product', 'name sku');
        res.status(201).json({
