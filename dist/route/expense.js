...
router.get("/", auth, async (req, res) => {
    try {
        const { page = 1, limit = 20, search, category, subcategory, status, startDate, endDate, paymentMethod, isRecurring, tags } = req.query;
...
router.get("/records", auth, async (req, res) => {
    try {
        const { page = 1, limit = 20, search, category, startDate, endDate, status, tags } = req.query;
        const pageNum = typeof page === 'number' ? page : parseInt(page) || 1;
        const limitNum = typeof limit === 'number' ? limit : parseInt(limit) || 20;
        const skip = (pageNum - 1) * limitNum;
        const filters = { createdBy: req.user._id };
