...
router.post("/withdraw", auth, async (req, res) => {
    try {
        ...
        try {
            ...
            const transferResult = await PaystackService.initiateTransfer(recipientCode, amount, description);
...
router.post("/transfer", auth, async (req, res) => {
    try {
        ...
        // Get sender's wallet
        const senderWallet = await Wallet.getUserWallet(new mongoose.Types.ObjectId(req.user._id));
...
router.post("/create-recipient", auth, async (req, res) => {
    try {
        ...
        // Create transfer recipient
        const result = await PaystackService.createTransferRecipient("nuban", name, accountNumber, bankCode);
        if (result.success) {
            res.json({
