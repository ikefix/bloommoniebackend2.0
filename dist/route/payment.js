...
router.post("/initialize", auth, async (req, res) => {
    try {
        ...
        const result = await PaystackService.initializeTransaction(user.email, amount, metadata);
        if (result.success) {
...
router.post("/verify/:reference", auth, async (req, res) => {
    try {
        ...
        // Get user wallet
        const wallet = await Wallet.getUserWallet(new mongoose.Types.ObjectId(userId));
...
router.post("/webhook", async (req, res) => {
    try {
        ...
        if (event === 'charge.failed') {
            ...
            await LedgerEntry.updateMany({ gatewayReference: reference, status: "pending" }, {
                status: "failed",
