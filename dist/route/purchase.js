...
router.post("/", auth, async (req, res) => {
    try {
        ...
        if (purchase.status === "received") {
            await purchase.receivePurchase(new mongoose.Types.ObjectId(req.user._id));
        }
...
router.get("/", auth, async (req, res) => {
    try {
        const { page = 1, limit = 20, search, supplier, status, startDate, endDate } = req.query;
        ...
        if (search) {
            query = Purchase.searchPurchases(search, new mongoose.Types.ObjectId(req.user._id), filters);
        }
...
router.get("/orders", auth, async (req, res) => {
    try {
        ...
        if (endDate)
            filters.orderDate = { $lte: new Date(endDate) };
        const orders = await PurchaseOrder.getPendingOrders(new mongoose.Types.ObjectId(req.user._id), filters)
            .skip(skip)
            .limit(limitNum);
...
router.get("/history", auth, async (req, res) => {
    try {
        const { page = 1, limit = 20, supplier, status, startDate, endDate } = req.query;
        const pageNum = typeof page === 'number' ? page : parseInt(page) || 1;
        const limitNum = typeof limit === 'number' ? limit : parseInt(limit) || 20;
        const skip = (pageNum - 1) * limitNum;
        const filters = {
...
router.get("/summary", auth, async (req, res) => {
    try {
        ...
        const summary = await Purchase.getPurchaseSummary(new mongoose.Types.ObjectId(req.user._id), startDate, endDate);
        res.json({
            success: true,
            data: summary[0] || {
                totalPurchases: 0,
...
router.get("/overdue", auth, async (req, res) => {
    try {
        const overduePurchases = await Purchase.getOverduePurchases(new mongoose.Types.ObjectId(req.user._id));
        res.json({
            success: true,
