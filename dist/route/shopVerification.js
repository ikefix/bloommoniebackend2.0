...
router.post("/add-cashier", auth, async (req, res) => {
    try {
        ...
        await sendEmail(email, "Shop Access Invitation - Bloomrest", emailTemplates.shopInvitation(name || email.split('@')[0], invitationLink, shop.shopName));
        res.status(200).json({
        ...
        await sendEmail(email, "Welcome to Bloomrest - Shop Access", emailTemplates.shopVerification(name, verificationCode, verificationLink, shop.shopName));
        res.status(201).json({
