...
import Purchase, { IPurchaseDocument } from "../models/purchase.js";
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const purchase = new Purchase(purchaseData) as unknown as IPurchaseDocument;
    await purchase.calculateTotals();
    await purchase.save();
    
    // Update stock if purchase is received
    if (purchase.status === "received") {
      await purchase.receivePurchase(new mongoose.Types.ObjectId(req.user._id as string));
    }
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (search) {
      query = Purchase.searchPurchases(search as string, new mongoose.Types.ObjectId(req.user._id as string), filters);
    }
...
router.put("/:id", auth, async (req, res) => {
  try {
    const purchase = await Purchase.findOne({ 
      _id: req.params.id, 
      createdBy: req.user._id 
    }) as unknown as IPurchaseDocument | null;
...
router.get("/orders", auth, async (req, res) => {
  try {
    ...
    const orders = await (PurchaseOrder as any).getPendingOrders(new mongoose.Types.ObjectId(req.user._id as string), filters)
      .skip(skip)
      .limit(limitNum);
...
router.post("/:id/return", auth, async (req, res) => {
  try {
    ...
    for (const returnItem of returnItems) {
      ...
      await (product as any).updateStock(returnItem.quantity, "subtract");
    }
...
router.get("/summary", auth, async (req, res) => {
  try {
    ...
    const summary = await Purchase.getPurchaseSummary(new mongoose.Types.ObjectId(req.user._id as string), startDate as string, endDate as string);
...
router.get("/overdue", auth, async (req, res) => {
  try {
    const overduePurchases = await Purchase.getOverduePurchases(new mongoose.Types.ObjectId(req.user._id as string));
