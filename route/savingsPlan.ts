...
import SavingsPlan, { ISavingsPlanDocument } from "../models/savingsPlan.js";
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const savingsPlans = await (SavingsPlan as any).getUserSavingsPlans(req.user._id, status);
  ...
  try {
    ...
    res.json({
      savingsPlan: {
        ...
        availableBalance: (savingsPlan as any).availableBalance,
        autoSave: savingsPlan.autoSave,
        withdrawalRules: savingsPlan.withdrawalRules,
        contributions: (savingsPlan as any).contributions?.map((c: any) => ({
          amount: c.amount,
          reference: c.reference,
          description: c.description,
          createdAt: c.createdAt
        })) || [],
        withdrawals: (savingsPlan as any).withdrawals?.map((w: any) => ({
          amount: w.amount,
...
router.post("/:id/contribute", auth, async (req, res) => {
  try {
    ...
    // Add contribution to savings plan
    await (savingsPlan as unknown as ISavingsPlanDocument).addContribution(amount, reference, description || "Savings contribution");
...
router.post("/:id/withdraw", auth, async (req, res) => {
  try {
    ...
    // Request withdrawal
    const withdrawal = await (savingsPlan as unknown as ISavingsPlanDocument).requestWithdrawal(amount, description || "Savings withdrawal");
    ...
    res.json({
      ...
      withdrawal: {
        id: (withdrawal as any)._id,
        amount: (withdrawal as any).amount,
        reference: (withdrawal as any).reference,
        description: (withdrawal as any).description,
        penaltyFee: (withdrawal as any).penaltyFee,
...
router.get("/:id/transactions", auth, async (req, res) => {
  try {
    ...
    if (!type || type === "contribution") {
      transactions.push(...(savingsPlan as any).contributions?.map((c: any) => ({
        type: "contribution",
        amount: c.amount,
        reference: c.reference,
        description: c.description,
    ...
    if (!type || type === "withdrawal") {
      transactions.push(...(savingsPlan as any).withdrawals?.map((w: any) => ({
        type: "withdrawal",
        amount: w.amount,
        reference: w.reference,
        description: w.description,
...
router.get("/stats/overview", auth, async (req, res) => {
  try {
    const savingsPlans = await (SavingsPlan as any).getUserSavingsPlans(req.user._id);
