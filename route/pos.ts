...
router.get("/customers/search", auth, async (req, res) => {
  try {
    ...
    const customers = await (Customer as any).searchCustomers(query, new mongoose.Types.ObjectId(req.user._id as string));
...
router.get("/discounts", auth, async (req, res) => {
  try {
    const discounts = await (Discount as any).findValidDiscounts(new mongoose.Types.ObjectId(req.user._id as string));
...
router.post("/cart/discount", auth, async (req, res) => {
  try {
    ...
    const discount = await (Discount as any).getByCode(discountCode, new mongoose.Types.ObjectId(req.user._id as string));
...
router.get("/sales/summary", auth, async (req, res) => {
  try {
    ...
    const summary = await (Sale as any).getSalesSummary(new mongoose.Types.ObjectId(req.user._id as string), startDate, endDate);
...
router.get("/sales", auth, async (req, res) => {
  try {
    ...
    let query = (Sale as any).searchSales(search, new mongoose.Types.ObjectId(req.user._id as string), filters);
