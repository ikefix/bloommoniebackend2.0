...
router.delete("/products/:id", auth, async (req, res) => {
  try {
    ...
    product.lastUpdatedBy = new mongoose.Types.ObjectId(req.user._id as string);
    await product.save();
...
router.get("/units", auth, async (req, res) => {
  try {
    const units = await (Unit as any).getAllUnits();
...
router.get("/stock/movements/:productId", auth, async (req, res) => {
  try {
    ...
    const pageNum = typeof page === 'number' ? page : parseInt(page as string) || 1;
    const limitNum = typeof limit === 'number' ? limit : parseInt(limit as string) || 50;
    const skip = (pageNum - 1) * limitNum;
    ...
    const movements = await StockMovement.getProductMovements(
      new mongoose.Types.ObjectId(req.params.productId as string), 
      filters
...
router.get("/stock/transfer/pending", auth, async (req, res) => {
  try {
    const transfers = await (StockTransfer as any).getPendingTransfers();
...
router.post("/stock/transfer/:id/approve", auth, async (req, res) => {
  try {
    ...
    await (transfer as any).approveTransfer(req.user._id);
    await StockTransfer.findById(transfer._id).populate('items.product', 'name sku');
...
router.post("/stock/transfer/:id/complete", auth, async (req, res) => {
  try {
    ...
    await (transfer as any).completeTransfer(req.user._id);
    await StockTransfer.findById(transfer._id).populate('items.product', 'name sku');
...
router.post("/stock/transfer/:id/cancel", auth, async (req, res) => {
  try {
    ...
    await (transfer as any).cancelTransfer(reason);
...
router.get("/stock/transfer/history", auth, async (req, res) => {
  try {
    ...
    const transfers = await (StockTransfer as any).getTransferHistory(filters);
  ...
  try {
    const [
      ...
      Product.countDocuments({ 
        isActive: true, 
        "stock.currentStock": { $gt: 0, $lte: (mongoose.Schema.Types.Decimal128 as any)("$stock.reorderPoint") }
      }),
      Product.countDocuments({ 
        isActive: true, 
