...
import Wallet, { IWalletDocument } from "../models/wallet.js";
import LedgerEntry, { ILedgerEntryDocument } from "../models/ledger.js";
...
router.get("/balance", auth, async (req: Request, res: Response) => {
  try {
    const wallet = await Wallet.getUserWallet(new mongoose.Types.ObjectId(req.user._id as string));
...
========================= */
router.post("/add-money", auth, async (req: Request, res: Response) => {
  try {
    ...
    const fraudCheck = await (FraudDetection as any).analyzeTransaction({
      userId: req.user._id,
      ipAddress: req.ip,
    ...
    await (LedgerEntry as any).createDoubleEntry([
      {
        accountType: "wallet",
...
router.post("/transfer", auth, async (req: Request, res: Response) => {
  try {
    ...
    // Get sender's wallet
    const senderWallet = await Wallet.getUserWallet(new mongoose.Types.ObjectId(req.user._id as string));
    ...
    await (LedgerEntry as any).createDoubleEntry([
      {
        ...
        metadata: {
          ...
          recipientPhone: (recipient as any).phone,
          fraudCheckId: fraudCheck._id
        },
        ipAddress: req.ip,
        userAgent: req.get('User-Agent')
    ...
    res.json({ 
      ...
      transaction: {
        ...
        toPhone: (recipient as any).phone,
...
router.get("/transactions", auth, async (req: Request, res: Response) => {
  try {
    ...
    // Build query
    const query: any = { userId: req.user._id, status: "completed" };
    ...
    const populatedTransactions = await Promise.all(
      transactions.map(async (transaction) => {
        const tx = transaction as unknown as ILedgerEntryDocument;
        const populated = { ...tx.toObject() };
        
        if (tx.metadata.recipientId) {
          const recipient = await User.findById(tx.metadata.recipientId).select('name phone');
          (populated as any).recipientDetails = recipient;
        }
        
        if (tx.metadata.senderId) {
          const sender = await User.findById(tx.metadata.senderId).select('name phone');
          (populated as any).senderDetails = sender;
        }
    ...
    res.json({
      transactions: populatedTransactions.map(t => ({
        ...
        amount: (t as any).credit || (t as any).debit,
        description: (t as any).description,
        balance: (t as any).balance,
        metadata: (t as any).metadata,
        createdAt: (t as any).createdAt,
        recipientDetails: (t as any).recipientDetails,
        senderDetails: (t as any).senderDetails
      })),
      pagination: {
        currentPage: pageNum,
...
========================= */
router.post("/create-recipient", auth, async (req: Request, res: Response) => {
  try {
...
/* =========================
   GET BANKS LIST
========================= */
router.get("/banks", auth, async (req: Request, res: Response) => {
  try {
...
========================= */
router.post("/resolve-account", auth, async (req: Request, res: Response) => {
  try {
...
/* =========================
   LOCK/UNLOCK FUNDS
========================= */
router.post("/lock-funds", auth, async (req: Request, res: Response) => {
  try {
...
router.post("/unlock-funds", auth, async (req: Request, res: Response) => {
  try {
