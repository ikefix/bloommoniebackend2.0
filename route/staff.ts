...
import Staff, { IStaffDocument } from "../models/staff.js";
...
router.get("/accounts", auth, async (req, res) => {
  try {
    ...
    if (search) {
      query = (Staff as any).searchStaff(search as string, filters);
    }
...
router.get("/roles", auth, async (req, res) => {
  try {
    const roles = await (Role as any).getRolesWithPermissions();
...
router.get("/roles/level/:level", auth, async (req, res) => {
  try {
    ...
    const roles = await (Role as any).getRolesByLevel(levelValue);
...
router.get("/permissions", auth, async (req, res) => {
  try {
    const permissions = await (Permission as any).getAllPermissions();
...
router.get("/permissions/:module", auth, async (req, res) => {
  try {
    ...
    const permissions = await (Permission as any).getPermissionsByModule(module);
...
router.get("/performance", auth, async (req, res) => {
  try {
    ...
    let performance = await (Staff as any).getStaffSalesPerformance(startDate, endDate);
...
router.post("/performance/:id", auth, async (req, res) => {
  try {
    ...
    await (staff as unknown as IStaffDocument).updatePerformance(performanceData);
...
router.get("/sales-report", auth, async (req, res) => {
  try {
    ...
    const performance = await (Staff as any).getStaffSalesPerformance(startDate, endDate);
