...
import Sale, { ISaleDocument } from "../models/sale.js";
...
router.post("/credit", auth, async (req, res) => {
  try {
    ...
    const sale = new Sale(saleData) as unknown as ISaleDocument;
...
router.post("/:id/return", auth, async (req, res) => {
  try {
    ...
    for (const returnItem of returnItems) {
      ...
      await (product as any).updateStock(returnItem.quantity, "add");
    }
...
router.post("/quotations", auth, async (req, res) => {
  try {
    ...
    const quotation = new Invoice(quotationData) as unknown as IInvoiceDocument;
