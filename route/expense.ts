...
import {
  ...
  ProcessRecurringExpensesRequest,
  GetExpensesQuery,
  GetExpenseCategoriesQuery,
  GetExpenseSummaryQuery,
  GetExpenseTrendsQuery
...
router.get("/categories", auth, async (req: Request, res: Response) => {
  try {
    const categories = await ExpenseCategory.getCategoryTree(new mongoose.Types.ObjectId(req.user._id));
...
// Get root categories
router.get("/categories/root", auth, async (req: Request, res: Response) => {
  try {
    const categories = await ExpenseCategory.getRootCategories(new mongoose.Types.ObjectId(req.user._id));
...
router.post("/categories", auth, async (req: Request, res: Response) => {
  try {
    const categoryData: CreateExpenseCategoryRequest = {
      ...req.body,
...
router.get("/categories/search", auth, async (req: Request, res: Response) => {
  try {
    const { search }: GetExpenseCategoriesQuery = req.query;
    
    if (!search) {
      return res.status(400).json({ message: "Search query is required" });
    }
    
    const categories = await ExpenseCategory.searchCategories(search, new mongoose.Types.ObjectId(req.user._id));
...
router.get("/", auth, async (req: Request, res: Response) => {
  try {
    ...
    if (startDate) filters.expenseDate = { $gte: new Date(startDate as string) };
    if (endDate) filters.expenseDate = { $lte: new Date(endDate as string) };
    
    let query = Expense.find(filters);
    
    if (search) {
      query = Expense.searchExpenses(search, new mongoose.Types.ObjectId(req.user._id), filters);
    }
...
router.put("/:id", auth, async (req: Request, res: Response) => {
  try {
    ...
    Object.assign(expense, updateData, { updatedBy: req.user._id });
    await expense.save();
...
router.get("/recurring", auth, async (req: Request, res: Response) => {
  try {
    const { 
      page = 1, 
      limit = 20,
    ...
    const expenses = Expense.getRecurringExpenses(new mongoose.Types.ObjectId(req.user._id))
...
router.post("/recurring/process", auth, async (req: Request, res: Response) => {
  try {
    const { expenseIds }: ProcessRecurringExpensesRequest = req.body;
...
router.get("/pending", auth, async (req: Request, res: Response) => {
  try {
    ...
    const expenses = await Expense.getPendingExpenses(new mongoose.Types.ObjectId(req.user._id))
      .skip((pageNum - 1) * limitNum)
      .limit(limitNum);
...
router.post("/:id/approve", auth, async (req: Request, res: Response) => {
  try {
    ...
    await expense.approve(new mongoose.Types.ObjectId(req.user._id));
...
// Reject expense
router.post("/:id/reject", auth, async (req: Request, res: Response) => {
  try {
    const { reason }: RejectExpenseRequest = req.body;
    ...
    await expense.reject(new mongoose.Types.ObjectId(req.user._id), reason);
...
// Reimburse expense
router.post("/:id/reimburse", auth, async (req: Request, res: Response) => {
  try {
    const expense = await Expense.findOne({ 
      _id: req.params.id, 
      createdBy: req.user._id 
    ...
    await expense.reimburse(new mongoose.Types.ObjectId(req.user._id));
...
router.get("/summary", auth, async (req: Request, res: Response) => {
  try {
    const { startDate, endDate, category }: GetExpenseSummaryQuery = req.query;
    
    const summary = await Expense.getExpenseSummary(new mongoose.Types.ObjectId(req.user._id), startDate, endDate);
    
    // Filter by category if specified
    let filteredSummary = summary;
    if (category) {
      filteredSummary = await Expense.getExpensesByCategory(new mongoose.Types.ObjectId(req.user._id), startDate, endDate);
    }
...
router.get("/by-category", auth, async (req: Request, res: Response) => {
  try {
    const { startDate, endDate }: GetExpenseSummaryQuery = req.query;
    
    const categoryExpenses = await Expense.getExpensesByCategory(new mongoose.Types.ObjectId(req.user._id), startDate, endDate);
...
router.get("/trends", auth, async (req: Request, res: Response) => {
  try {
    const { year = new Date().getFullYear() }: GetExpenseTrendsQuery = req.query;
