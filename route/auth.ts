...
import { LoginUserRequest, RegisterUserRequest } from "../types/auth.type.js";
...
router.post("/register", async (req: Request, res: Response) => {
  try {
    const { name, email, password, phone, termsAndConditionsAccepted }: RegisterUserRequest = req.body;
    ...
    try {
      ...
      await newShop.save();
      console.log('Default shop created successfully for new user:', email);
    } catch (shopError) {
      console.error('Error creating default shop for new user:', shopError);
      // Don't throw - allow user creation to proceed even if shop creation fails
    }
...
router.post("/login", async (req, res) => {
  try {
    ...
    const uniqueShops = allAccessibleShops.filter((shop, index, self) =>
...
========================= */
router.get("/open-in-app", (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user) {
      ...
      try {
        ...
        const newShop = new shop(shopData);
        await newShop.save();
        console.log('Default shop created successfully for Google user:', user.email);
      } catch (shopError) {
        console.error('Error creating default shop for Google user:', shopError);
        // Don't throw - allow user creation to proceed even if shop creation fails
      }
    ...
    // For Electron apps, always use deep link scheme
    const scheme = process.env.APP_DEEP_LINK_SCHEME || 'bloommonie';
    const deepLinkCallbackUrl = `${scheme}://google-callback?code=${code}&token=${token}&userId=${user._id}`;

    // Always redirect to deep link for Electron/desktop apps
    res.redirect(deepLinkCallbackUrl);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
router.get("/me", auth, async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findById((req as any).user.id).select('-password -verificationToken -otp -otpCreatedAt -resetPasswordToken -resetPasswordTokenExpiry');

    const shops = await shop.find({ createdBy: (req as any).user.id });
    
    // Also find shops where user is in allowedUsers
    const allowedShops = await shop.find({ allowedUsers: (req as any).user.id });
...
router.post("/google/signin", async (req: Request, res: Response) => {
  try {
    ...
    // Find or create user
    let user: any = await User.findOne({ email: tokenInfo.email });
...
/* =========================
   ADMIN CREATE USER
========================= */
router.post("/create-user", auth, async (req: Request, res: Response) => {
  try {
...
router.get("/public/all-users", async (req: Request, res: Response) => {
  try {
