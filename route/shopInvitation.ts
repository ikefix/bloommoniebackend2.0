...
router.post("/accept-invitation", async (req, res) => {
  try {
    ...
    await sendEmail(
      ...
      (emailTemplates as any).welcome(newUser.name, (shop as any).shopName)
    );
  ...
  try {
    ...
    if (!user) {
      ...
      await sendEmail(
        (user as any).email,
        "Welcome to Bloomrest!",
        (emailTemplates as any).welcome(user.name, (shop as any).shopName)
      );
    } else {
      // Add existing user to shop if not already added
    ...
    const jwtToken = jwt.sign(
      { id: user._id, email: (user as any).email },
