...
router.post("/add-cashier", auth, async (req, res) => {
  try {
    ...
    const invitationLink = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/shop-invitation?token=${invitationToken}&shopId=${shop._id}&shopName=${encodeURIComponent((shop as any).shopName)}`;
    
    // Send invitation email
    await sendEmail(
      email,
      "Shop Access Invitation - Bloomrest",
      emailTemplates.shopInvitation(name || email.split('@')[0], invitationLink, (shop as any).shopName)
    );
    ...
    await sendEmail(
      ...
      emailTemplates.shopVerification(name, verificationCode, verificationLink, (shop as any).shopName)
    );
...
router.post("/verify-shop-access", async (req, res) => {
  try {
    ...
    (user as any).termsAndConditionsAccepted = true;
    await user.save();
