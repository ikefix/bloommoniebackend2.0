...
import VirtualAccount, { IVirtualAccountDocument } from "../models/virtualAccount.js";
...
router.post("/create", auth, async (req, res) => {
  try {
    ...
    // Create virtual account
    const virtualAccount = await (VirtualAccount as any).createVirtualAccount(
      req.user._id,
      accountName,
      provider
    );
...
router.get("/", auth, async (req, res) => {
  try {
    const virtualAccount = await (VirtualAccount as any).getUserVirtualAccount(new mongoose.Types.ObjectId(req.user._id as string));
...
router.delete("/deactivate", auth, async (req, res) => {
  try {
    ...
    await (VirtualAccount as any).deactivateVirtualAccount(new mongoose.Types.ObjectId(req.user._id as string));
...
router.post("/webhook", async (req, res) => {
  try {
    ...
    if (event === 'charge.success') {
      ...
      const va = virtualAccount as unknown as IVirtualAccountDocument;
  ...
  try {
    ...
    res.json({
      transactions: transactions.map(t => ({
        ...
        amount: (t as any).credit,
        description: (t as any).description,
        gatewayReference: (t as any).gatewayReference,
