...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (search) {
      query = (Invoice as any).searchInvoices(search as string, req.user._id, filters);
    }
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    await (invoice as unknown as IInvoiceDocument).calculateTotals();
    await invoice.save();
...
router.post("/from-sale", auth, async (req, res) => {
  try {
    ...
    const invoice = new Invoice(invoiceData);
    await (invoice as unknown as IInvoiceDocument).calculateTotals();
    await (invoice as unknown as IInvoiceDocument).sendInvoice(new mongoose.Types.ObjectId(req.user._id as string));
...
router.post("/:id/payment", auth, async (req, res) => {
  try {
    ...
    await (invoice as unknown as IInvoiceDocument).addPayment(amount, paymentMethod, reference);
...
router.get("/overdue", auth, async (req, res) => {
  try {
    const overdueInvoices = await (Invoice as any).getOverdueInvoices(req.user._id);
...
router.get("/summary", auth, async (req, res) => {
  try {
    ...
    const summary = await (Invoice as any).getInvoiceSummary(req.user._id, startDate, endDate);
