...
router.post("/verify/:reference", auth, async (req, res) => {
  try {
    ...
    // Get user wallet
    const wallet = await Wallet.getUserWallet(new mongoose.Types.ObjectId(userId as string));
    ...
    for (const entry of pendingEntries) {
      ...
      (entry as any).reference = verifiedReference;
      await entry.save();
    }
    ...
    if (result.success) {
      ...
      res.json({
        ...
        data: {
          ...
          internalStatus: ledgerEntry ? (ledgerEntry as any).status : "not_found"
        }
      });
    } else {
      res.status(400).json({
    ...
    res.json({
      ...
      data: {
        transactions: transactions.map(t => ({
          ...
          paymentMethod: (t as any).metadata?.paymentMethod || "unknown",
        ...
        try {
          ...
          if (!existingTransaction) {
            ...
            if (pendingEntries.length > 0) {
              ...
              for (const entry of pendingEntries) {
                (entry as any).status = "completed";
                (entry as any).reference = webhookReference;
                await entry.save();
              }
